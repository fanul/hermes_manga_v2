...
def process_title(title, log):
    ...
    # Step 3: CHECK if there's already an active download queue
    pre_queue = get_queue_info()
    if pre_queue and (pre_queue.get("queue_size", 0) > 0 or pre_queue.get("active", 0) > 0):
        log(f"   ⏳ Queue already active (size={pre_queue['queue_size']}, active={pre_queue['active']}) — marking download_pending, retry next tick")
        return "download_pending", manga_id, 0, score

    # Step 4: enqueue all chapters
    enqueue_chapters_bulk(chapter_ids)

    # Step 5: start downloader
    start_downloader()

    # Step 6: post-enqueue verify
    time.sleep(2)
    post_queue = get_queue_info()
    if post_queue is not None and post_queue["queue_size"] == 0 and post_queue["active"] == 0:
        log(f"   ✅ All chapters already downloaded")
...
def main():
    ...
    def save_and_advance(title, status, manga_id=None, num_chapters=0, score=0, reason=""):
        ...
        # download_pending = don't advance, retry next tick
        if status != "download_pending":
    ...
    while budget_ok() and state["current_index"] < len(titles):
        ...
        if existing in ("done", "kavita_skip", "not_found", "downloading", "download_pending"):
            if existing in ("downloading", "download_pending"):
                ...
                # For download_pending: do NOT advance — same title should retry next tick
                # For downloading: advance — we already enqueued, let scan handle completion
                if existing == "download_pending":
                    save_state(state)
